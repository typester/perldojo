...
配列の中にある値があるかどうかの真偽値を返す C<in($value, @array)> を実装することのしまし。
...
4はわかりにくいのですが、 C<map> ブロックの評価値はリストコンテキストで評価されるので、値を増やしたり減らしたりできるのです。つまり C< map { expr || () } ... > は C<grep> と同じ意味になります。
