...
C<$string>はUTF8フラグ付きの文字列である。この文字列をUTF8を表示できる環境でエラーや warning を出さずに表示したい。正しいものを選べ。
...
C<$string>に、UTF8フラグが付いていても、その文字列が UTF8でデコードされているかは、C<utf8::is_utf8>では、判断できません。
...
そして、UTF8でデコードされていないので、文字列の長さは 15 となる。
なお、この文字列をUTF8フラグがあるからといって、C<Encode::encode('utf8', $string);> を行うと、文字化けしてしまいます。
